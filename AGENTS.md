...
- **DEFAULT_CHAT_SYSTEM_PROMPT** in `core/constants.py` (imported by `main.py`, `chat_panel.py`) instructs the model to: (1) use **get_markdown** to read (full or range) and **apply_markdown** to write — for "replace whole document" (e.g. make my resume look nice) call `get_markdown(scope="full")` once, then `apply_markdown(markdown=<new markdown>, target="full")` and pass **only the new content**, never the original document text; for partial replace use `target="range"` with start/end; (2) use internal linguistic knowledge for translate/proofread/edit — NEVER refuse translation; (3) no preamble, concise reasoning, one-sentence confirmation after edits.
